...
async function handleWordFile(file) {
  ...
  fd.append("word", file);
  try {
    const data = await postForm("/reports/api/upload", fd);
...
$("btn-revalidate").addEventListener("click", async () => {
  ...
  try {
    const report = await postJSON("/reports/api/validate_template", { word_id: state.wordId });
...
$("btn-preview").addEventListener("click", async () => {
  ...
  try {
    const preview = await postJSON("/reports/api/preview", { excel_id: state.excelId, year: +year, month: +month });
    ...
    if (state.fillReception && state.receptionId) {
      const rec = await postJSON("/reports/reports/api/preview_reception", { reception_id: state.receptionId });
...
function renderResult(summary, filename, outputId) {
  ...
  dl.href = `/reports/api/download/${outputId}?filename=${encodeURIComponent(filename)}`;
...
$("btn-generate").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await postJSON("/reports/api/generate", payload);
